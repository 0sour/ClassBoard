...
|------|------------------------|---------------------------|--------------------------|
| 周课表页 | **双日视图**（v2.8）：默认显示今天+明天两列（周日只显示当天），左右滑动逐日切换（左滑→更晚一天，右滑→更早一天），推入式滑动动画；顶部为双日标题条（日期标题 + 今天胶囊按钮），隐藏周导航；禁用长按拖拽；行高 56px | 显示 7 列，行高 52px | 显示 7 列，行高 64px；右侧信息面板固定 296px |
...
- 手机端（<768px）为**双日视图**：默认显示今天+明天两列（周日只显示当天），左右滑动逐日切换（pointer 事件，水平位移 >50px 判定），推入式滑动动画（Transition + enter 绝对定位覆盖，新旧同时动画）；顶部为双日标题条（日期标题 + 今天胶囊按钮），隐藏周导航；禁用长按拖拽（避免与滑动冲突）`[已实现]`（v2.8，会话 34-38）。桌面/平板显示周一至周日 7 天；节次行高：桌面 64px / 平板 52px / 移动 56px `[Data-backed]`（PRD 5.2）；跨节次课程合并行渲染 `[已实现]`。
