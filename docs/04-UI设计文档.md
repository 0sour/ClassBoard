...
| 4 | 课程编辑弹窗 | 弹窗 | 详情弹层内「编辑」（`[未实现]`）；周视图空白格点击（`[未实现]`）；顶栏「导入课程 → 手动录入」 | 双端 | `[部分实现]`（手动录入可用，详情内编辑/空白格新增未做） |
| 5 | PDF 课表导入流程 / 手动录入弹窗 | 向导（4 步）或手动录入弹窗 | 顶栏「导入课程」下拉（从 PDF 导入 / 手动录入） | 双端 | `[已实现]`（实际为 4 步：上传→解析预览→确认导入→完成） |
| 6 | 事项页（考试 / 实验 / 作业三子 Tab 分类列表） | 页面 | 移动端底部 Tab「事项」；桌面顶栏入口 | 双端 | `[部分实现]`（三 Tab 骨架可用：实验列表真实数据；考试为空态占位；作业为本地 mock 勾选，见 3.5） |
| 7 | 设置页 | 页面 | 移动端底部 Tab「设置」；桌面顶栏齿轮 | 双端 | `[部分实现]`（学期切换/编辑、节次只读展示；新建/删除学期、节次编辑、提醒、备份、口令设置未做，见 3.6） |
| 8 | 访问口令页 | 页面 | 开启口令后未认证进入 | 双端 | `[未实现]`（后端接口已实现，见 3.7） |
...
- 课程色块悬停：`--shadow-card` 增强为 `--shadow-hover` 并 `translateY(-1px)`（`--motion-duration-normal` + `--motion-easing-standard`），按压回位；点击色块打开详情弹层 `[已实现]`；点击空白格直接进入新增课程弹窗，预填星期与起止节次 `[未实现]`。
...
- 单双周过滤默认开启，当前周为单周只显示单周与全部周课程 `[已实现]`（关闭过滤的开关在设置页 `[未实现]`，store 已具备 showOddEvenFilter 状态）。
- 两个课程时间重叠时，后导入者向右错位半格显示，色块右上角加 `--color-feedback-warning` 小圆点标记，详情中标注冲突（见第 4 节「冲突错位」）`[已实现]`。
- 周课表页操作区提供「打印」与「导出 PNG」操作，入口同时置于桌面顶栏图标位，操作对象仅限课表网格（第 4.7 节）`[未实现]`。
...
线框说明：A 区为日期导航，左右箭头切换日期，「今天」按钮回当日（实际实现为标题 + 当日课程数，日期导航的上一日/下一日切换 `[未实现]`）；B 区为当日摘要，用数字概览课程、考试、作业数量，移动端右侧常驻铃铛角标入口（未读时显示 `--color-feedback-warning` 色角标数字），点击展开站内提醒下拉/列表，提醒项覆盖上课提醒与作业截止提醒（开启后）——**B 区摘要条与铃铛角标 `[未实现]`**，实际实现为「今日作业」区块（mock 数据，按当天星期过滤课程 id 或 courseId=null 展示，`[已实现]` 但数据未接真实接口）；C 区是按时间排序的课程卡片，卡片头部为课程色圆点与课程名 `[已实现]`；D 区为挂在课程下的考试、作业条目，点击后直接打开对应编辑弹窗（PRD 5.2）`[未实现]`。桌面端 B 区摘要并入右侧信息面板（`[未实现]`）。
...
线框说明：详情弹层自上而下为四段。A 区头部左侧 10px 圆点取该课程色板强调色（`--course-{n}-text`），视觉归属课程 `[已实现]`；B 区以键值对形式展示课程全部字段（键 `--color-text-tertiary`、值 `--color-text-body` `--font-weight-medium`，备注值置灰底 `--color-bg-page` + 边框 `--color-border-default` 圆角块）`[已实现]`；C 区用 Tab 聚合该课程的考试与作业（PRD 5.7 编辑入口之一）`[未实现]`；D 区为操作栏，删除为危险操作，触发确认对话框 `[未实现]`（当前详情弹层为纯只读，编辑/删除入口见《开发日志》待办）。

编辑弹窗复用同一结构：A 区变为「新增课程 / 编辑课程」标题，B 区替换为表单（课程名称、**课程类型（普通课/实验课，默认普通课）**、教师、地点、颜色选择、周次、星期、起止节次、备注）`[已实现]`（颜色选择 `[未实现]`，当前按轮询规则自动分配）；D 区变为「取消 / 保存 / 保存并继续」（`--color-brand` 主按钮、`--shadow-btn`）`[部分实现]`（当前为「取消 / 保存课程」，「保存并继续」未实现）。

「保存并继续」用于手动录入：保存成功后表单清空已保存字段，预填上一门课的常用字段（星期、地点、颜色），停留在弹窗内可直接录入下一门 `[未实现]`；「保存」保存后关闭弹窗 `[已实现]`。手动录入直接调用课程创建接口，不经过 PDF 解析与预览流程，与 PDF 导入互相独立 `[已实现]`。
...
线框说明：考试、实验、作业共用一套列表骨架，由 A 区三个子 Tab 切换 `[已实现]`。A 区标题区分三类，右侧为新增按钮（新增按钮按当前子 Tab 变化：考试/作业为对应记录弹窗，实验为新增实验课课程弹窗）`[未实现]`；B 区提供课程与状态筛选（实验子 Tab 无状态筛选）`[未实现]`；C 区列表：考试与作业按时间倒序 `[未实现]`（考试子 Tab 当前为空态占位「还没有考试安排」，作业子 Tab 为本地 mock 数据、勾选仅存页面状态无持久化），实验子 Tab 展示课程类型=实验课的课程（按星期、节次排序）`[已实现]`（数据来自 store.visibleCourses，点击打开课程详情弹层）；D 列表项展示名称、所属课程标签、时间与状态。桌面端点击列表项时，D 展开为右侧详情面板（两栏布局）`[未实现]`。
...
- 考试与作业均按时间倒序；作业项内嵌勾选控件，勾选后条目文字加删除线（`line-through`），状态即时切换无需保存 `[部分实现]`（勾选交互可用，但数据为本地 mock、无持久化，未接 `/api/homework`）。
- 实验子 Tab：展示课程类型=实验课的课程列表（课程名、教师、地点、星期与节次），点击条目打开课程详情弹层（第 3.3 节），从详情可编辑或删除该实验课课程（`[未实现]`，详情弹层只读）；列表页「新增」打开课程编辑弹窗并预选「实验课」类型 `[未实现]`。
- 点击条目打开编辑弹窗；考试字段：课程、名称、日期时间、地点、备注；作业字段：课程、名称、截止时间、完成状态、备注 `[未实现]`。
- 删除条目需二次确认 `[未实现]`。
...
线框说明：设置页分六组。A 区为分组导航，移动端为单列分组列表逐项展开，桌面为左侧分组导航 + 右侧设置面板 `[部分实现]`（当前为单页三个 panel：学期 / 节次时间模板 / 其他，无桌面左导航）。B 至 G 为各设置面板：B 学期管理展示学期列表（学期名称、起止日期、当前学期标记），支持新建、编辑、删除与切换当前学期 `[部分实现]`（切换 `PUT /api/semesters/current` 与行内编辑起止日期已实现；新建、删除学期 `[未实现]`）；C 节次时间模板为可编辑起止时间列表，默认 12 节（含两节晚自习），行数可增删，删除行后节次序号自动重排（校验结束晚于开始）`[未实现]`（当前为只读展示全部节次，后端 periods CRUD 已实现）；D 提醒设置含课程提醒总开关（默认关闭，首次使用时经引导开启）、实验课提醒独立开关（默认关闭，仅对课程类型=实验课的课程生效）、提醒时机（每节课 / 仅当天首课）、提前分钟数（5/10/15/30）与浏览器通知授权状态提示，作业截止提醒为独立开关（默认关闭）`[未实现]`（后端设置字段已实现）；E 数据管理提供 JSON 导出与导入 `[未实现]`（当前为「开发中」文案占位，后端 backup 接口已实现）；F 访问口令为 4–20 位 `[未实现]`（后端 access 接口已实现）；G 外观固定浅色，`color-scheme: light` 锁定，本期不提供主题切换（设计语言第 11 节）`[已实现]`（当前以「外观：固定清新浅色」占位行呈现）。
...
线框说明：口令页为全屏居中卡片布局，双端形态一致。A 区为整页背景 `--color-bg-page`；B 区卡片垂直水平居中，使用 `--color-bg-surface` 底色 + `--shadow-pop` 投影 + `--radius-xl` 圆角，自上而下依次为品牌 logo（28px 圆角图标底 `--color-brand-subtle` + `--color-brand` 图标）与「ClassBoard」应用名；C 区口令输入框对齐表单规范（`--color-bg-subtle` 输入框底、`--color-border-default` 边框、`--radius-sm`、聚焦 `--color-border-focus` 描边），密码掩码显示，长度校验 4–20 位；D 区确认按钮使用 `--color-brand` 背景、`--color-text-inverse` 文字、`--shadow-btn` 投影；口令校验失败时 E 区在输入框下方显示 `--color-feedback-error` 色文案（如「口令不正确」），输入框边框同步标红。移动端竖屏保持卡片居中，卡片宽度适配视口并适配安全区，不启用底部弹层。

> **实现状态：`[未实现]`**（v2.2 标注）——本页尚未开发，后端 4 个 access 接口（verify/logout/enable/disable）已实现并通过测试；前端 `bootstrap` 已识别 `accessRequired` 降级响应并保持 mock 模式，等待口令页落地（见《开发日志》待办）。
...
> **实现状态：`[未实现]`**（v2.2 标注）——无全局 Toast 组件；当前反馈以内联表单错误条（`role="alert"`）与导入向导步骤页呈现，全局 Toast 规范留待后续统一落地（见《开发日志》待办）。
...
> **实现状态：`[未实现]`**（v2.2 标注）——无通用确认对话框组件；覆盖导入的警告目前以向导内提示条呈现，删除类二次确认待实现（见《开发日志》待办）。
...
> **实现状态：`[未实现]`**（v2.2 标注）——当前无骨架屏组件；前端启动以 mock 数据先行渲染、remote 数据到达后替换（store 双模式），加载态未做骨架屏。
...
- 用户可在课程编辑弹窗「颜色选择」区手动改色，改色仅作用于当前课程，后续新增同名课程仍跟随轮询默认色 `[未实现]`（编辑器无颜色选择控件，见《开发日志》待办）。
- 颜色区分布依赖视觉层级：色块浅底描边 + 课程名深字，改色不影响可读性（设计语言第 12 节）`[已实现]`。

### 4.7 课表打印与导出 PNG

> **实现状态：`[未实现]`**（v2.2 标注，见《开发日志》待办）
...
> **实现状态：`[未实现]`**（v2.2 标注）——当前 `index.html` 仅含 `theme-color` meta，Web App Manifest 未注册；`public/icons.svg` 图标资产已就位，等待 manifest + 192/512 图标落地（见《开发日志》待办）。
