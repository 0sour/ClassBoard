...
| 文档版本 | v2.10 |
| 创建日期 | 2026-08-13 |
| 更新说明 | v2.0 依据「清新浅色极简」风格（风格预览 01）定稿视觉体系，全部 token 与布局细节对齐《03-设计语言文档》v2.0；v2.1 依据开发细节确认：移动端底部 Tab 导航、手机始终 7 天、节次可增删（默认 12 节含晚自习）、多学期管理、提醒默认关闭，并补充站内铃铛、课程配色轮询、PWA、打印导出、删学期回退、覆盖导入语义、口令页、横屏策略、手动录入课程（并入导入入口下拉、保存并继续）、实验课类型与独立实验课提醒、考试/实验/作业三子 Tab 分类列表；课程导入改为 PDF 课表导入（正方教务系统导出 PDF，前端解析），移除 Excel 导入。v2.2 依据 2026-08-14 代码盘点同步真实实现：页面清单合并事项页、移动端底部 Tab 实为 4 项（考试/作业并入「事项」）、周导航「未开学/假期」徽标、AppSelect 外部关闭改为文档级 mousedown 监听（无遮罩）；各设计项以 `[已实现]` / `[部分实现]` / `[未实现]` 标注实际落地状态（未实现项见《开发日志》待办）。v2.3 依据会话 22 UI 完善第一批同步：事项页/设置页/口令页/课程编辑删除/颜色选择/保存并继续/单双周开关/备份/Toast/Confirm/打印导出/PWA 全部落地。v2.4 依据会话 23 同步：新增 4.10 时间选择（TimePicker）控件——全项目时间选择统一使用自研 TimePicker（不使用原生 time/datetime-local），节次模板改为竖排双列卡片 + 行内编辑 + 时间冲突校验。v2.5 依据会话 24 同步：新增 4.11 日期选择（DatePicker）控件——全项目日期选择统一使用自研 DatePicker（不使用原生 date），与 TimePicker 同架构（Teleport + fixed 跟随）；桌面顶栏补事项页入口。v2.6 依据会话 25 同步：周视图空白格快捷新增课程、课程详情考试/作业 Tab、骨架屏（4.4）、事项页桌面双栏右侧详情面板（3.5）全部落地。v2.7 依据会话 26+27 同步：冲突课程视图从左右 50%/50% 错位改为 1/3⇄2/3 动态 flex 分栏（初始均分、hover 展开、最后停留保留、rAF 流畅动画）；空槽按钮 gridColumn:1 修复隐式列；时间列 grid-template-columns 恢复单列；日视图与信息面板「今日课程」独立数据源修复；信息面板摘要卡与周导航按钮行上边缘对齐（margin-top: spacing-md）；待交作业与实践与其他合并为 seg 切换组合卡；节次模板卡片双列竖排（4.9 实现状态更新）。v2.8 依据会话 28-38 同步：实验课 lab-tag 随课程色板（--ctext/--cline，全胶囊）；实践课程无固定时间（unscheduled 标志，详情显示「无固定时间」）；长按拖拽调课（单周例外拆分）；移动端周课表改为双日视图（默认今天+明天、左右滑动逐日切换、推入式动画、周日单日、禁用拖拽、隐藏周导航改双日标题条+今天按钮）；移动端顶栏适配（学期限宽、导入图标化）；审计修复（weekInfoOf 周号漂移、remote 颜色基准、404 兜底、maxWeeks 30 周上限、exportPng Firefox 兼容等）。v2.9 依据会话 39-40 同步：新增 4.12 城市搜索下拉（CitySearchSelect）控件——天气设置城市输入改用搜索下拉（300ms 防抖、QWeather 城市库、省/市归属、键盘导航），并新增 4.13 天气卡组件说明。v2.10 依据会话 41-42 同步：**移动端周课表从双日视图改为单日视图**——默认显示今天一天（全宽课程卡片列表），顶部为**轮盘式日期选择器**（固定 201 格以今天为中心的序列 + 原生横向滚动 + 常驻中央指示器 + 松手自动回正），替代原双日左右滑动切日；周标题/今天按钮保留；详见 4.3 周课表页与 4.14 日期轮盘` |
...
| 3 | 课程详情弹层 | 弹层 | 点击课程色块 / 课程卡片 | 双端 | `[已实现]`（编辑/删除入口 `[已实现]`，会话 22；考试/作业 Tab `[已实现]`，会话 25） |
...
页面依赖关系：周视图与日视图共享同一份课程数据与节次时间模板；课程详情弹层从两个视图均可打开；考试、作业的编辑弹窗可从列表页与课程详情弹层两处进入（PRD 5.7）；各学期数据相互独立，学期切换后全局数据刷新为所选学期。站内提醒入口不单独成页：桌面端为顶栏铃铛按钮（`[已实现]`，数据来自真实接口：今日课程 + 未完成作业），移动端铃铛角标 `[未实现]`，点击展开的提醒列表为轻量下拉/弹层。
...
|------|------------------------|---------------------------|--------------------------|
| 周课表页 | **单日视图**（v2.10）：顶部日期标题（周标题 + 周次/单双周徽标 + 今天胶囊按钮，可点周标题打开周选择器）；其下为**日期轮盘**（201 格以今天为中心，横向滚动选日，常驻中央指示器，松手自动回正）；再下为当天课程全宽卡片列表；隐藏周导航；禁用长按拖拽；行高 56px | 显示 7 列，行高 52px | 显示 7 列，行高 64px；右侧信息面板固定 296px |
...
| D 课程色块 | `CourseBlock` | 底色 `--course-{n}-bg`、描边 `--course-{n}-line`、课程名 `--course-{n}-text`、地点 `--color-text-secondary`；`--radius-sm`、外边距 `--spacing-2xs`、内边距 `--spacing-sm`；悬停 `--shadow-hover` + `translateY(-1px)`；实验课（type=lab）色块右上角叠加「实验」小标签（白底 + 卡片自身 `--ctext` 文字 + `--cline` 描边 + `--radius-full` 全胶囊 + `--font-size-xs` 字号，随课程色板）`[已实现]`（v2.7：从固定紫色改为跟随卡片色板 token，修复硬编码 font-size/圆角违规）。冲突课程（type=conflict）右上角叠加 8px warning 圆点标记 `[已实现]`。两者并存时冲突圆点位于左上角避免重叠。 |
...
- 手机端（<768px）为**单日视图**（v2.10）：默认显示今天一天（全宽课程卡片列表，按节次排序），顶部为**日期轮盘**选日（见 4.14），周标题点击弹出周选择器、附周次/单双周徽标，右侧「今天」胶囊按钮一键回到今天；隐藏周导航；禁用长按拖拽 `[已实现]`（v2.10，会话 41-42；替代 v2.8 双日视图）。桌面/平板显示周一至周日 7 天；节次行高：桌面 64px / 平板 52px / 移动 56px `[Data-backed]`（PRD 5.2）；跨节次课程合并行渲染 `[已实现]`。
...
- 两个课程时间重叠时，按 1/3⇄2/3 动态 flex 分栏显示（初始均分、hover 展开最后停留的课程为 2/3、折叠态仅显示课程名、rAF 补间动画），色块右上角加 `--color-feedback-warning` 小圆点标记（见第 4.5 节「冲突分栏」）`[已实现]`（v2.7 从错位半格改为 flex 分栏）。
- 周课表页操作区提供「打印」与「导出 PNG」操作，入口同时置于桌面顶栏图标位，操作对象仅限课表网格（第 4.7 节）`[已实现]`（会话 22：周视图操作条「打印」+「导出 PNG」，打印 `@media print` A4 横向仅输出网格与周导航条）。

**信息面板（右侧栏）**

- 桌面端为课表右侧固定 296px 栏，`position: sticky; top: calc(var(--topbar-h) + var(--spacing-md))`（72px，与周导航内容行上边缘对齐）`[Expert judgment]`，与课表同屏滚动；平板折为两列排布在课表下方（两张卡各占一列）；手机单列。卡片间间距 `--spacing-lg`。
...
- 信息面板底部为**组合卡**：卡内顶部为分段控件（样式同顶栏视图切换 seg：托盘 `--color-bg-subtle`、`--radius-md`、内边距 3px，激活段白底 + `--shadow-card` + 主色文字），两段「待交作业」与「实践与其他」切换显示列表，两段等宽均分 `[会话 26]`；「待交作业」段 tab 带数量角标（`--radius-full`、`--font-size-xs`，非激活态 `--color-brand-subtle` 底主色字，激活态 `--color-brand` 底白字），列表按截止时间倒序，空态显示「暂无待交作业，好好休息」；「实践与其他」段列出实践类课程条目（名称 + 教师与周次元信息）。
...
线框说明：考试、实验、作业共用一套列表骨架，由 A 区三个子 Tab 切换 `[已实现]`。A 区标题区分三类，右侧为新增按钮（新增按钮按当前子 Tab 变化：考试/作业为对应记录弹窗，实验为新增实验课课程弹窗）`[已实现]`（会话 22）；B 区提供课程与状态筛选（实验子 Tab 无状态筛选）`[部分实现]`（作业 Tab 状态筛选已实现，课程筛选未做）；C 区列表：考试与作业按时间倒序 `[已实现]`（会话 22 接入真实接口），实验子 Tab 展示课程类型=实验课的课程（按星期、节次排序）`[已实现]`；D 列表项展示名称、所属课程标签、时间与状态 `[已实现]`。桌面端点击列表项时，D 展开为右侧详情面板（两栏布局）`[已实现]`（会话 25：≥1280px 左侧列表 + 右侧 sticky 详情面板，选中项高亮，编辑保存后刷新选中态）。
...
### 4.5 课程色块冲突错位显示（1/3⇄2/3 动态分栏）

> **实现状态：`[已实现]`**（v2.7 更新，会话 26+27：从左右 50%/50% 改为 flex 动态分栏）

- 触发条件：同一时段（同日、同节次区间重叠）存在两门及以上课程（PRD 5.2）`[已实现]`。
- 布局规则：冲突课程组渲染为 flex 容器（`.overlap`），列内 `grid-template-columns: 1fr` 单列；容器内按 `flex-grow` 分配宽度 `[已实现]`。
- **初始状态（从未 hover）**：两门课程均分（`flex-grow: 1:1`），各占列宽约 50%，两门都显示完整信息（课程名 + 地点）`[已实现]`。
- **hover 交互**：鼠标 hover 到某门课程时，该课程展开为 `flex-grow: 2`（约列宽 66%），另一门收缩为 `flex-grow: 1`（约列宽 33%）；**移开鼠标后保留最后停留的课程为 2/3 展开** `[已实现]`。
- **折叠态**：`flex-grow: 1` 的课程设置 `collapsed` prop（`getGrow(id) < maxGrow - 0.01`），仅显示课程名（字号 `--font-size-xs`，垂直居中），隐藏地点 `[已实现]`；展开态显示课程名 + 地点 `[已实现]`。
- **动画**：flex-grow 变化通过 JS `requestAnimationFrame` + easeOutCubic 补间驱动（200ms），不依赖 CSS `transition`（避免 IAB 渲染器冻结 CSS 动画）`[已实现]`。
- 冲突课程组的分组算法：按重叠关系（`isOverlap`）求连通分量，每组渲染一个 `.overlap` 容器；组内按节次+id 排序保证稳定 `[已实现]`。
- 两门以上同时冲突时，flex-grow 仍为 2:1:1…（展开的 2/3，其余均分 1/3）`[已实现]`。
- 课程色块右上角保留 `--color-feedback-warning` 圆点标记（冲突指示）`[已实现]`。
- 详情弹层中冲突课程以警示条标注「与 XX 课程时间重叠」`[未实现]`。
- 列内空槽按钮（`.slot`）显式设置 `gridColumn: 1`，避免触发隐式列 `[已实现]`（会话 26 修复）。
...
### 4.12 城市搜索下拉（CitySearchSelect）控件

**选型**：天气设置的城市输入使用自研组件 `CitySearchSelect`（`frontend/src/components/common/CitySearchSelect.vue`），不使用普通文本输入。城市需要从和风天气城市库选择（保证城市名可被 API 正确解析），搜索下拉同时展示城市归属省/市，避免同名城歧义。

**结构**：文本输入框（可输入城市名）+ 下拉结果列表 + 文档级 `mousedown` 外部关闭（与 AppSelect 一致）。

**交互**：

- 输入即搜索：**300ms 防抖**后请求 `GET /api/weather/cities?q=<关键字>`（服务端代理 QWeather 城市搜索，24h 缓存，最多 8 条结果）；输入框右侧显示旋转加载图标（`--color-text-tertiary`，`animation: spin 0.8s linear infinite`）。
- 下拉列表：城市名（`--font-weight-medium`、`--color-text-body`）+ 省/市归属（`--font-size-xs`、`--color-text-tertiary`，如「浙江省 · 杭州」）；hover 与键盘激活项 `--color-brand-subtle` 底。
- 键盘：↑/↓ 移动选中（循环）、Enter 选中、Esc 关闭；选中后关闭下拉并把城市名写入值。
- 空态提示：无结果显示「未找到匹配城市，试试「杭州」「信阳」等名称」；未输入显示「输入城市名搜索，如「杭州」」。
- 选中城市后值只保存城市名（如「杭州」），服务端按城市名解析经纬度/城市 ID。

**样式**：

- 输入框：高 34px、宽 ≥220px，`--color-bg-surface` 底 + `--color-border-default` 边框 + `--radius-sm` + `--font-size-sm`；聚焦与展开时边框 `--color-border-focus`。
- 下拉面板：`top: calc(100% + 6px)`、`min-width: 100%`、`max-width: 320px`、`max-height: 260px`（内部滚动），`--color-bg-surface` + `--color-border-default` + `--radius-md` + `--shadow-pop` + `padding: var(--spacing-xs)`，层级 `--z-index-float`；入场 `opacity + translateY(-4px)`（`--motion-duration-normal`），出场 `opacity`（`--motion-duration-fast`）。

**无障碍**：输入框 `aria-label`（天气城市）、`autocomplete="off"`；下拉 `role="listbox"`、选项 `role="option"` + `aria-selected`；选项按钮 `mousedown.prevent` 防输入框失焦。

**使用范围**：设置页天气面板城市选择；项目内任何需要「城市搜索」的场景一律复用 `CitySearchSelect`。

### 4.14 日期轮盘（DayWheel）控件

**选型**：移动端周课表的日期选择使用自研轮盘控件（`WeekView.vue` 内 `dv-wheel`，不抽独立组件），替代原生 `input[type=date]`/下拉列表——轮盘一屏展示多日、横向滑动惯性手感与 iOS 日期选择器一致，且与单日课程列表天然联动。桌面端不出现（桌面用周导航 + 7 列网格）。

**结构**（自上而下）：

- 标题行：周标题（日历图标 + 「周三 · 8月26日」+ 周次/单双周徽标，点击弹出 WeekPicker）+ 右侧「今天」品牌色实心胶囊按钮。
- 轮盘：`dv-wheel` 原生横向滚动容器（高 52px、隐藏滚动条、`touch-action: pan-x`），内部 201 个日期格（`dv-wheel__day`，宽 56px：星期小字 + 日期数字）；中央为常驻指示器 `dv-wheel__indicator`（品牌色 1.5px 圆角边框，绝对定位固定在滚动容器外的 wrap 上，对齐视口中央，轨道从框内流过）。

**数据与状态**：

- 固定序列：以今天为中心 ±100 天（201 格，`WHEEL_CENTER=100`），序列永不重建、无回滚；`wheelIdx`（中央格索引）为唯一状态源，日期标题与课程列表均由它派生（`activeDay`）。
- 滚动映射：`scroll` 事件 rAF 节流读 `scrollLeft / 56` 四舍五入为索引，实时更新选中日期（拖动过程标题/列表即时跟随，方向按新旧日期先后判定）。
- 点击格/「今天」按钮：`scrollTo({ behavior: 'smooth' })` 平滑滚动，滚动过程日期实时跟随。

**自动回正**：滚动停止 300ms 后（滚动事件计时器兜底，兼容无 `scrollend` 的环境）平滑吸附到最近整格，使选中日期恰好停在指示器中央；吸附结束 420ms 后再校验一次并同步索引；指针按下/取消时清除定时器，拖动中不吸附。`wheelSettling` 标志防止吸附滚动再触发自身调度。

**不使用 `scroll-snap`**：浏览器 snap 与 JS 吸附双重作用会产生「无法选中其他日期、总回弹」的冲突，因此关闭 CSS scroll-snap，吸附完全由 JS 负责。

**切日动画**：课程列表以 `activeDay` 为 key，按方向（next/prev）推入式滑入滑出（`Transition` out-in，200ms）。

**样式**：格宽 56px；星期 `--font-size-xs`/`--color-text-tertiary`，日期数字 `--font-size-md`/`--font-weight-medium`；今天数字品牌色加粗；选中格文字品牌色加粗（高亮框由固定指示器提供，格子本身不亮，避免拖动时高亮跟随格子的错位感）；轨道两端 `padding: 0 calc(50% - 28px)` 使首尾格也能滚到中央。

**无障碍**：日期格为 `<button>`（`type="button"`），滚动容器 `user-select: none`；指示器为纯视觉装饰（`aria-hidden`）。
