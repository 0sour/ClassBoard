...
| 36 | POST /api/templates | 创建模板（admin；kind=course/unit） |
| 37 | PUT /api/templates/:id | 更新模板（admin；version+1） |
| 38 | DELETE /api/templates/:id | 删除模板（admin；自动清理 unit 引用） |
| 39 | POST /api/templates/:id/import | 导入单个模板（复制快照到目标学期） |
...
| kind | TEXT | NOT NULL，course/unit | course=单门课程模板（1-N 行，固定课表或每节课调整）；unit=组合模板（引用课程模板 id 数组或课程行快照） |
...
| content | TEXT | NOT NULL，JSON | course：课程行数组；unit：课程模板 id 数组（引用）或课程行数组（快照） |
