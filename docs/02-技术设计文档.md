...
| 实现状态 | v1.1 依据 2026-08-14 代码盘点同步真实实现：后端接口/数据层已全部实现并通过 8 项集成测试；前端 62 项单测全绿；正文以 `[已实现]` / `[部分实现]` / `[未实现]` 标注各设计项的实际落地状态。v1.2 依据会话 22 UI 完善第一批同步：事项页/设置页/口令页/课程编辑删除/提醒引擎/打印导出/PWA 等前端模块落地；v1.3 依据会话 31-38 同步：Docker 部署完成（NAS 192.168.10.110:3005）、移动端双日视图、审计修复等；v1.4 依据会话 41-42 同步：移动端周课表改为单日视图 + 日期轮盘（固定 201 格序列、原生滚动映射、松手自动回正），详见《04-UI设计文档》4.14 与 3.3.1；未实现项见《开发日志》待办 |
...
graph TB
    ...
    subgraph 容器["classboard 容器（node:22-alpine）"]
...
| 选型 | 理由 | 服务的需求 |
|------|------|------------|
| node:22-alpine 单容器 | 与前端构建产物同进程托管，镜像约 50-70 MB，Alpine 在 NAS 常见（含 ARM）上镜像拉取快；better-sqlite3@13 要求 Node≥22 | PRD 6 部署（x86/ARM、NAS） |
...
**3.3.1 应用骨架与状态管理** `[已实现]`：Vue Router 四个视图——`/week`（周视图）、`/day`（今天）、`/matters`（事项：考试/实验/作业三 Tab）、`/settings`（设置），`/` 重定向到 `/week`；路由级动态导入使各视图按需加载。状态管理为**单一 `scheduleStore`**（`stores/schedule.ts`，与文档早期「四个 store」设想不同，实际合并为 1 个）：持有学期列表/当前学期/节次/课程/周上下文、考试/作业全量列表与设置（含提醒、访问口令状态）；其中 `scheduleStore` 持有聚合接口返回的周数据作为渲染唯一来源，避免多接口拼装导致的时序不一致。视图按当前学期拉取数据，设置页包含学期管理（新建/删除/切换/编辑全部 `[已实现]`，会话 22）。新课程颜色分配在前端 `scheduleStore` 完成：按录入顺序从 8 色预设中轮询，课程名（name）相同的复用已有颜色，后端仅存色值 `[已实现]`；课程颜色手动修改（编辑器 8 色色卡）`[已实现]`（会话 22）。`index.html` 注册 Web App Manifest（含 192/512 图标）`[已实现]`（会话 22：`public/manifest.webmanifest` + PIL 生成 PNG 图标）。周视图打印与导出 PNG `[已实现]`（会话 22：`@media print` A4 横向打印样式 + `utils/exportPng.ts` DOM→canvas 2x 高清导出）。

**3.3.2 提醒引擎** `[已实现]`（会话 22，`utils/reminder.ts`）：页面打开时启动（App 挂载），每 30s 基于当日课程（`coursesByWeekday` + 节次模板）与提醒设置检查：为每节满足条件（提醒时机=每节课，或仅当天首课）的课程计算「开始时间 − 提前分钟数」，到点调用 Notification API（权限未授予时不发起，站内铃铛兜底——TopBar 铃铛下拉展示今日课程 + 未完成作业）；课程提醒按课程类型分流：普通课由 `reminder` 设置控制，实验课（type='lab'）由独立的 `labReminder` 设置控制，两者互不影响、均默认关闭。站内铃铛入口：桌面端为顶栏图标（未读角标）；提醒内容含上课与作业截止两类（作业按 `homeworkReminder.advanceDays` 在截止前 N 天每天提示一次）；提醒数据由前端基于 store 数据生成，无需额外接口。已触发提醒按「课程 id + 日期 + 类型」去重，页面关闭后定时器随页面销毁（无服务端推送）。
...
**3.3.4 主题与断点**：主题固定为清新浅色（`color-scheme: light`，本期无暗色模式，见设计语言文档 v2.2 第 11 节）；断点工具返回设备类别供默认视图跳转与周课表布局切换使用（手机端为单日视图 + 日期轮盘，桌面/平板为 7 天网格，PRD 5.2）。
...
| 31 | GET /api/backup | 导出全量 JSON（含模板） |
| 32 | POST /api/backup/restore | 导入 JSON 恢复（含模板，仅 admin） |
| 33 | GET /api/settings | 读设置 |
| 34 | PUT /api/settings | 写设置 |
| 35 | GET /api/templates | 模板列表（含导入统计） |
| 36 | POST /api/templates | 创建模板（admin；kind=course/unit/semester） |
| 37 | PUT /api/templates/:id | 更新模板（admin；version+1） |
| 38 | DELETE /api/templates/:id | 删除模板（admin；自动清理 unit 引用） |
| 39 | POST /api/templates/:id/import | 导入单个模板（复制快照到目标学期；semester 模板自动创建学期+节次） |
| 40 | POST /api/templates/import-batch | 批量导入多个课程模板 |
...
**template（课程模板 / 组合模板，全局共享）**

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | INTEGER | PK，自增 | 模板主键 |
| kind | TEXT | NOT NULL，course/unit/semester | course=单门课程模板（1-N 行，固定课表或每节课调整）；unit=组合模板（引用课程模板 id 数组或课程行快照）；semester=学期模板（学期信息 + 节次时间） |
| name | TEXT | NOT NULL，1-50 | 模板名称 |
| category | TEXT | NOT NULL，默认空 | 分类（已不在 UI 展示，字段保留兼容） |
| description | TEXT | NOT NULL，默认空 | 描述 |
| version | INTEGER | NOT NULL，默认 1 | 版本号，编辑时 +1 |
| content | TEXT | NOT NULL，JSON | course：课程行数组；unit：课程模板 id 数组（引用）或课程行数组（快照）；semester：`{name, startDate, endDate, weekStartDay, periods:[{startTime,endTime}]}` |
| created_by | INTEGER | NOT NULL，FK → user(id) ON DELETE CASCADE | 创建者 |
...
| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | INTEGER | PK，自增 | 记录主键 |
| template_id | INTEGER | NOT NULL，FK → template(id) ON DELETE CASCADE | 导入的模板 |
| template_version | INTEGER | NOT NULL | 导入时的模板版本 |
| semester_id | INTEGER | NOT NULL | 导入目标学期 |
| user_id | INTEGER | NOT NULL，FK → user(id) ON DELETE CASCADE | 导入用户 |
| mode | TEXT | NOT NULL | append / overwrite / dedupe |
| count | INTEGER | NOT NULL | 实际导入课程数 |
| imported_at | TEXT | NOT NULL | ISO 时间 |

模板设计要点：**复制（快照）语义**——导入即复制课程行到用户学期，模板后续修改不影响已导入者；unit 引用形态下管理员修改课程模板后组合自动生效（导入时展开引用）；删除课程模板时自动从引用它的 unit 中移除该 id。
...
> **实现状态：`[已实现]`**（2026-08-19 会话 31 完成，提交 `1767d00`；已部署至 NAS 192.168.10.110:3005，Debian 12 x86_64 + Docker 29.6.1 + Compose v5.3.1）。实际实现与设计稿差异：基础镜像为 `node:22-alpine`（better-sqlite3@13 要求 Node≥22）；构建阶段走宿主机 clash 代理（`build.network: host` + `HTTP(S)_PROXY=http://127.0.0.1:7890`，NAS 直连外网不稳定）；compose 增加 `CLASSBOARD_DATA_DIR=/app/data` 环境变量（数据库落盘目录）；实际端口 3005（3000/3001 被 NAS 其他服务占用）。

### 7.1 镜像构建

多阶段构建要点：

- 构建阶段（`node:22-alpine`）：安装 `python3 make g++`，复制前后端 package.json 与 lockfile 执行 `npm ci`，构建前端产出 `dist/`（Vite 配置 `outDir` 指向后端静态目录），并编译 better-sqlite3 原生模块；前端构建产物含 Web App Manifest 与图标（192/512），由静态托管层直接提供，不做 Service Worker 离线缓存；mupdf 为前端构建依赖，其 WASM（约 10 MB）随 Vite 打包进 `dist/` 且按需懒加载，运行阶段无需任何服务端 PDF 依赖；若目标平台有预编译产物（prebuild-install 自动拉取）则跳过编译，否则源码编译兜底 `[To be confirmed]`（依赖 better-sqlite3 对 musl 的预编译覆盖情况）。
- 运行阶段（`node:22-alpine`）：`NODE_ENV=production`，仅复制后端源码、依赖（`npm ci --omit=dev`）与前端构建产物，创建 `/app/data` 并 `chown` 到非 root 用户，`USER node` 运行；`EXPOSE 3000`、`VOLUME /app/data`、启动命令 `node server/index.js`。
...
services:
  classboard:
    ...
    build:
      ...
      # 构建阶段复用宿主网络栈：Dockerfile 内 npm/node-gyp 经 127.0.0.1:7890 代理拉取
      network: host
    ports:
      - "${CLASSBOARD_PORT:-3000}:3000"
    environment:
      - TZ=${TZ:-Asia/Shanghai}
      # 数据库落盘目录（server/lib/db.js 支持环境变量覆盖；与下方数据卷对应）
      - CLASSBOARD_DATA_DIR=/app/data
    volumes:
      - ./data:/app/data
