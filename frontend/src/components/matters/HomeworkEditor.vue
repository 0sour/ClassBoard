...
const props = defineProps<{ open: boolean; homework: Homework | null }>()
...
watch(
  ...
  (v) => {
    ...
    busy.value = false
    if (props.homework) {
      form.courseId = props.homework.courseId
      form.name = props.homework.name
      form.date = props.homework.dueAt.slice(0, 10)
      form.time = props.homework.dueAt.slice(11, 16)
      form.done = props.homework.done
      form.remark = props.homework.remark
    } else {
      form.courseId = null
      form.name = ''
      form.date = ''
...
async function submit(): Promise<void> {
  ...
  try {
    const payload = {
      ...
      name,
      dueAt: `${form.date}T${form.time}`,
      done: form.done,
      remark: form.remark.trim(),
    }
    if (props.homework) await store.updateHomework(props.homework.id, payload)
    else await store.addHomework(payload)
    emit('done')
    emit('close')
  } catch (e) {
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="open" class="modal-mask" @mousedown.self="emit('close')" @keydown="onKeydown">
        <div class="modal" role="dialog" aria-modal="true" :aria-label="homework ? '编辑作业' : '新增作业'">
          <div class="modal-head">
            <h3 class="m-title">{{ homework ? '编辑作业' : '新增作业' }}</h3>
          ...
          <div class="modal-body">
            <div class="form-grid">
              ...
              <label class="field field--full">
                <span class="field__label">作业名称</span>
                <input v-model="form.name" class="text-input" type="text" placeholder="如：第 3 章习题" maxlength="50" />
              </label>
              <label class="field field--full">
              ...
              <label class="field">
                <span class="field__label">完成状态</span>
                <div class="switch-row">
                  <input v-model="form.done" class="switch" type="checkbox" role="switch" aria-label="是否已完成" />
                  <span class="switch-label">{{ form.done ? '已完成' : '未完成' }}</span>
                </div>
              </label>
              <label class="field">
